...
- Közlekedési Hatóság Hungary: no license stated; the only term is the site-wide "Minden jog fenntartva ©" (all rights reserved) in the Impresszum, with nothing restricting reuse, caching, storage, or automated access — treated as Private-use under CC.1. Shipped on the §CC.2 fallback (sent 2026-05-10 to the Légügyi Felügyeleti Hatósági Főosztály, elapsed 2026-06-09, no reply). Residual exposure: "all rights reserved" is broader than the use made of it. Served verbatim (hu-kh): "Source data from the Közlekedési Hatóság (Hungarian Transport Authority), Magyarország Légijármű Lajstroma — https://www.kozlekedesihatosag.kormany.hu/hu/dokumentum/104604; publicly accessible with no specified license and treated as Private-use, normalized into this project schema without implying endorsement." Coverage cost: the register publishes no ICAO 24-bit hex, so its rows answer tail-number lookups only.
