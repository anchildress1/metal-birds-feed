...
export type FeedResponseRow = Omit<FeedRow, 'icao_hex' | 'registration_key'> & {
  // Present only on /feed/registration, and null there where the register publishes no Mode S
  // address — the caller learns the hex is unavailable rather than that the field is missing.
