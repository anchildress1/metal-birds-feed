...
4. **A hosted query API or search.** metal-birds-feed ships a per-source SQLite artifact (every canonical field is a queryable column — point lookups by `icao_hex` / `registration`, plus filter/sort on any indexed field) but hosts no query endpoint — no full-text search, no filtering service, no list API. Consumers query the artifact themselves (R2 binding, HTTP range reads, or a local copy). The private enrichment Worker (CC.4) is a by-`icao_hex` point lookup, not a query surface.
...
**CC.4 Private output only.** The maintained deployment writes normalized artifacts only to Ashley's private R2 bucket. Ashley-operated applications read R2 directly inside the same Cloudflare account. There is no hosted **public** read endpoint, public download, public query surface, or public dataset publication. A **private, authenticated** endpoint is not a public read API and remains permitted under this rule: the enrichment lookup Worker (`worker/`) serves a descriptive by-`icao_hex` point-lookup slice to a single authorized consumer application over a UUID bearer secret, rate-limited, server-side — no unauthenticated access, no bulk/enumeration/query surface, no full-artifact access. The prohibition is on _public_ (unauthenticated) surfaces; an authenticated private API for a named consumer is not one. Source-available code (Polyform Shield) lets third parties fork and self-host against their own R2 buckets and their own per-source source-use assessments.
