...
**CC.4 Private output only.** The maintained deployment writes normalized artifacts only to Ashley's private R2 bucket. Ashley-operated applications read R2 directly inside the same Cloudflare account. There is no hosted **public** read endpoint, public download, public query surface, or public dataset publication. A **private, authenticated** endpoint is not a public read API and remains permitted under this rule: the feed service (Cloud Run, serving one consolidated `feed.sqlite`) serves a descriptive point-lookup slice to a single authorized consumer application over a UUID bearer secret, rate-limited, server-side — no unauthenticated access, no bulk/enumeration/query surface, no full-artifact access. Two keys are exposed, `icao_hex` and a normalized registration, because most registers publish no Mode S address; both are batched exact lookups on a unique key, which is what keeps this inside the rule. Adding an attribute query, filter, or list surface would not be. The prohibition is on _public_ (unauthenticated) surfaces; an authenticated private API for a named consumer is not one. Source-available code (Polyform Shield) lets third parties fork and self-host against their own R2 buckets and their own per-source source-use assessments.
