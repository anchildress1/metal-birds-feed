...
- Redeploys on either an actual feed-data change or a new Release Please version. The scheduled `deploy-feed` job rebuilds the DB and deploys only if it differs from what is live (tracked by `_deployed.json`), while the release workflow deploys the exact released SHA after a release PR merge even when the feed data is unchanged. Ordinary code merges do not deploy before a release. A single source's failure never blocks shipping another source's update, and an all-unchanged refresh does not redeploy.
