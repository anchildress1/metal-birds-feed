...
- R2 keys (strict):
  ...
  - `aircraft/_translation_cache/<source>.json` — sha256(field+text) → English text, for Gemini delta translation of `cancellation_reason`/`airworthiness_class`/`lien_status` (not `idera_authorised_party`, which is a party name, not descriptive text). Persists independently of the artifact's content-hash skip gate.
- `src/localize/` translates the 3 free-text fields above, additive only (original never overwritten — a license, e.g. AESA Spain, can require the source meaning not be distorted). `localize.ts` dedupes by content hash before calling Gemini. A transient Gemini/cache failure leaves those fields `null` for the run without aborting the pipeline (enrichment, not a correctness gate); a missing `GEMINI_API_KEY` is a setup bug and deliberately throws instead.
