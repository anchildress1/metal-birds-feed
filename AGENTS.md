...
- R2 keys (strict):
  ...
  - `aircraft/_feed/<source>.json` — versioned per-source hex-collapsed slice (build intermediate). Cancelled records are excluded when the slice is written — once, in `toFeedRows`, since every slice comes from there. The feed enriches live traffic, and reissued marks are what would make `registration_key` ambiguous. A schema change to the slice is handled by deleting the slices so each source regenerates one (the cadence skip requires the slice to exist), never by filtering old shapes at merge. The per-source artifact still carries the full history — this is a feed-scope filter, not upstream loss. `main()` merges every slice into one `feed.sqlite` (`src/feed.ts`). Cadence-skip requires the slice to exist (self-heal, like the artifact); publish fails closed if any is missing. A bounded reader migration upgrades the pre-versioned shape and writes the current envelope back, so a failed source can still reuse its prior slice; unknown versions and malformed rows fail closed.
