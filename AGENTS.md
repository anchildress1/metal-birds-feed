...
- R2 keys (strict):
  ...
  - `aircraft/_translation_cache/<source>.json` — versioned envelope of sha256(field+text) → English text, for Gemini delta translation of `cancellation_reason`/`airworthiness_class` (not `idera_authorised_party` or `lien_status`, both of which can hold a party's name, not descriptive text). A contract-version mismatch invalidates the whole prior generation instead of retaining unreachable entries. Persists independently of the artifact's content-hash skip gate.
- `src/localize/` translates the 2 free-text fields above, additive only (original never overwritten — a license, e.g. AESA Spain, can require the source meaning not be distorted). `localize.ts` dedupes by content hash before calling Gemini. Any Gemini or cache failure leaves those misses `null` and never aborts the run — translation is enrichment, not a correctness gate. Only a missing `GEMINI_API_KEY` deliberately throws: that's a setup bug, not a runtime blip.
