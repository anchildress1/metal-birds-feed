...
- **Three onboarding surfaces, one audience each — a change to `make` targets, `.env.example`, or a required env var updates all three or none.** `docs/getting-started.md` (non-developer, manual) · `docs/getting-started-with-ai.md` (non-developer, agent-driven) · `.claude/skills/setup-metal-birds-feed/SKILL.md` (the agent itself). The skill is the only one an agent executes; the other two are human product documentation and keep human-facing conventions, so the AI-only style rule above does not apply to them.
- `.gitignore` ignores `.claude/*` and re-includes `.claude/skills/` so that skill ships in a clone. Ignoring `.claude/` instead silently kills the negation — git cannot re-include under an excluded directory — and the skill vanishes from every fork with nothing failing.
