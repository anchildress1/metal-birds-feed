...
- R2 keys (strict):
  ...
  - `aircraft/_translation_cache/<source>.json` — versioned envelope of sha256(field+text) → English text, for Gemini delta translation of `cancellation_reason`, `airworthiness_class`, `lien_status`, and `operational_classes` (each array element hashed individually). `idera_authorised_party` is never a candidate — it's always a party's NAME, not descriptive text. `lien_status` is excluded per-source rather than globally: mv-caa's `mortgage` cell holds the mortgagee's NAME (same problem as idera_authorised_party), but every other source's `lien_status` is a genuine description (e.g. br-anac's `DS_GRAVAME`) and is translated normally. A contract-version mismatch invalidates the whole prior generation instead of retaining unreachable entries. Persists independently of the artifact's content-hash skip gate.
