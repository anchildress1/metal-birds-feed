...
- R2 keys (strict):
  ...
  - `aircraft/_translation_cache/<source>.json` — versioned envelope of sha256(field+text) → English text, for Gemini delta translation of `cancellation_reason`/`airworthiness_class`/`lien_status` (not `idera_authorised_party`, which is a party name, not descriptive text). A contract-version mismatch invalidates the whole prior generation instead of retaining unreachable entries. Persists independently of the artifact's content-hash skip gate.
