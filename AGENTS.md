...
- `src/localize/` translates the 3 free-text fields above, additive only (original never overwritten — a license, e.g. AESA Spain, can require the source meaning not be distorted). `localize.ts` dedupes by content hash before calling Gemini. Exhausted transient Gemini failures leave those misses `null`; transient cache failures do not abort, while permanent Gemini/cache failures and a missing `GEMINI_API_KEY` deliberately throw.
