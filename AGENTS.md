...
- Source-published fleet total: `record_count.pattern` (regex, one capture group, matched against the decoded primary) — the engine asserts the translated count equals it and fails on mismatch, so a dropped row can't write a short artifact silently.
  - `record_count.url` matches a separate endpoint instead of the primary, for a register that publishes its total outside the file (data.gov.lt answers `?count()`). `pipeline.ts` fetches it after the register and passes the body to `translate`; the engine never touches the network. A publication landing between the two responses fails the run — rerun it rather than relaxing the check.
  - `record_count.against: parsed` counts every parsed row instead of the translated records. Required where the published total covers the accumulated history that `latest_snapshot_by` then filters, which is the only total an accumulating register states about itself.
  - Reach for the endpoint form on any paged API whose response cannot self-report truncation. Spinta closes every response with a `_page.next` cursor whether or not rows remain, so a short page is byte-identical in shape to a complete one; `MIN_RETAIN_RATIO` in `writer.ts` only catches a loss of more than half.
