...
- No _public_ (unauthenticated) read API. PRD §CC.4 forbids an unauthenticated one. `src/service/` complies: bearer auth, rate-limited, batched exact-`icao_hex` point lookup, one authorized consumer. Never add an attribute query/filter/list surface or full-artifact access.
...
- R2 keys (strict):
  ...
  - `aircraft/_feed/<source>.json` — versioned per-source hex-collapsed slice (build intermediate). `main()` merges every slice into one `feed.sqlite` (`src/feed.ts`). Cadence-skip requires the slice to exist (self-heal, like the artifact); publish fails closed if any is missing. A bounded reader migration upgrades the pre-versioned shape and writes the current envelope back, so a failed source can still reuse its prior slice; unknown versions and malformed rows fail closed.
