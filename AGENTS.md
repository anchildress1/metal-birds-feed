...
- The shipped skill lives in `.agents/skills/`, not `.claude/skills/`. `.agents/` is the vendor-neutral location Codex scans natively; `.claude/` stays fully gitignored as local agent scratch. Claude Code discovers only `~/.claude/skills/` and `.claude/skills/`, so it does **not** auto-load this one — both onboarding guides tell the reader to point their assistant at the path, with a symlink offered as the opt-in for either tool. Do not "fix" that by moving the skill back under `.claude/`; a gitignored home is what made it need a `!` negation in the first place.
